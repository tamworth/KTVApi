...
interface ISwitchRoleStateListener {
     ...
     * 加入合唱频道成功
     */
    fun onJoinChorusChannelSuccess(channel: String, uid: Int)
}
...
abstract class IKTVApiEventHandler {
     ...
     * 本地播放进度回调
     */
    open fun onPositionChanged(position_ms: Long, timestamp_ms: Long) {
 ...
 * @param chorusUid 合唱 uid
 * @param chorusChannelName 子频道名 加入子频道需要用到
 * @param chorusChannelToken 子频道token 加入子频道需要用到
 * @param maxCacheSize 最大缓存歌曲数
 * @param type 场景类型
 * @param isRoomOwner 是否是房主（根据业务选填）
 * @param ktvScene ktv的场景，0:49厅大合唱，伴唱角色用自己的pitch，1:KTV小厅用主持的pitch
 */
...
interface KTVApi {
     ...
     * 更新房间房主
     */
    fun upateRoomOwner(isRoomOwner: Boolean)
     ...
     */
    fun setLrcProgress(data: ByteArray?)

    fun setAudioMetadata(data: ByteArray?)
